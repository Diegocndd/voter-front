...
function PollDetail() {
  ...
  const url = `${process.env.REACT_APP_DOMAIN}/#/poll?id=${
  ...
  useEffect(() => {
    ...
    getPoll(idPoll)
      .then((res) => {
        ...
        getAlternatives(idPoll).then((result) => {
  ...
  useEffect(() => {
    ...
    if (limitDatePoll) {
      ...
      const localISODate = new Date(Date.now() - tzoffset)
        .toISOString()
        .slice(0, -1);
  ...
  const confirmDeletePoll = () => {
    deletePoll(window.location.hash.split("?")[1].split("=")[1]).then((res) => {
      if (res.status === 200) {
      ...
      {openModal ? (
        <div className="modal-delete-poll">
          <div style={{ marginBottom: 20 }}>
            <p id="title-modal-delete-poll">
              Tem certeza que deseja deletar a votação?
            </p>
            <p>
              Essa ação é irreversível! Você não terá mais acesso aos dados
              dessa votação.
            </p>
          </div>
          <div id="footer-buttons-container">
            <div id="cancel-button-modal" onClick={() => setOpenModal(false)}>
              Cancelar
            </div>
            <div id="delete-button-modal" onClick={() => confirmDeletePoll()}>
              Deletar
            </div>
          </div>
        </div>
        ...
        <div id="content-poll-detail">
          ...
          <div>
            {pollData.qty_votes === 0 ? (
              <p className="votes-poll-text">
                Sua votação ainda não possui nenhum voto!
              </p>
            ) : (
              <p className="votes-poll-text">
                Sua votação já possui <b>{pollData.qty_votes}</b>{" "}
                {pollData.qty_votes > 1 ? "votos" : "voto"}{" "}
              </p>
            )}
          </div>
          ...
          <div id="datelimit-poll">
            {isClosedDate ? (
              <p id="datelimit-poll-text" style={{ color: "#F01E1E" }}>
                <b>Votação encerrada!</b>
              </p>
            ) : (
              <p id="datelimit-poll-text">
                A votação encerrá dia <b>{formatDate(limitDate)}</b>
              </p>
            )}
          </div>
